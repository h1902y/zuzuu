...
> This is the part that makes zuzuu more than "files in a folder." The Project *grows* from how you work — and it grows safely, because **nothing is written without your yes.** This page is the loop that does it.
...
`observe` never touches the Project. It produces **proposals** — evidence-backed, typed changes (`create`, `update`, `delete`, `relate`, `deprecate`) that sit in a queue (`<module>/proposals/`). A proposal carries its `rationale`, its `evidence` (what justifies it — a command recurring across sessions, a hot file, corroborated past a **threshold** so a single coincidence never acts), a confidence, and a score. They're **ranked by score**, and **deduped** by content — re-proposing the same change does nothing, so the queue doesn't fill with noise.
...
## The gate: the one door to the Project

`review.mjs` is the only code that *writes* to the Project, and it only runs on your decision. Approving a proposal does three things atomically:
...
Each module pins **independently** — to roll the whole Project back, you roll each module's pointer. (A single whole-Project *checkpoint* composing every module into one pin was built, but it was never wired to any surface, so it was cut in the 2026-06-22 simplification pass rather than carried as dead code.)
