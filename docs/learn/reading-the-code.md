...
**6. The Project is written in exactly one place.** The tree is filed by concept (not by a strict dependency layer), so it's a plain DAG — no import cycles. The invariant that matters more than any layer diagram:

```
   use/        grow/                         the rule:
   query        observe → propose             only grow/ writes the Project,
   act          → review → snapshot           and only through review (the gate).
   check        + log (the feedback edge)      use/ only reads and runs.
```

`use/` and the rest never mutate your notes; every change to the Project flows through `grow/review.mjs`. (`act` does append a *run* to the git-ignored `runs.jsonl` via `grow/log` — the append-only feedback record — but that's not the Project.) Read a verb in `use/` and you know it can't rewrite your notes. An import cycle, or a note-write that skips the gate, is a bug — not a shortcut. (`tests/unit/architecture.test.mjs` pins both.)
...
2. **`use/`** — one file per thing you *do* to the Project: `query` (read), `act` (run), `check` (inspect).
3. **`grow/`** — how the Project *grows*, all in one place: `observe → propose → review → snapshot`, with `log` as the feedback edge.
...
- Does it keep the DAG acyclic, and route any write to the Project through `review` (rule 6)?
