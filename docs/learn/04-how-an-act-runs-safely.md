...
> Lesson `02` showed a runnable zu — an action, with `run` / `inputs` / `policy`. This page is about the scariest word in the project: **run**. Letting an agent execute commands is where things go wrong, so zuzuu is deliberately, honestly careful about it.

The code is `zuzuu/capabilities/act.mjs` (the runner) and `zuzuu/capabilities/gate.mjs` (the guardrails gate).
...
1. **A stored action** — `zz act build-report`. A *curated, reusable* zu the agent invokes. Each carries its own `policy`. This is `act.mjs`.
...
A zu's `policy.tier` says how contained the run is:
...
The key honesty rule, visible in the code: `act` reports `contained: true|false` truthfully. If a zu asks for `contained` but the sandbox backend isn't installed, it does **not** silently run uncontained pretending otherwise — it runs and flags `contained: false`. You always know the real tier.
...
One safety invariant: a zu's own `policy` can only **narrow** the module's default policy, never widen it. The module sets the ceiling; a runnable zu can tighten it.
...
1. **Run** — execute the zu's `run` (with inputs passed as `ZZ_*` env vars), under the tier's containment.
...
3. **Log** — append a `run` event to the module's `runs.jsonl`. The zu itself doesn't record the outcome (it stays pure definition); the module's log does.
...
`gate.mjs` is the guardrails capability. Its rules are just zus (`type: rule`) in the guardrails module — each with `action` (deny/ask/allow), `tool`, a `pattern`, and a `reason`. On every tool call, the gate matches the call against them; **severity wins** (deny beats ask beats allow); no match defers to the host's normal flow.
