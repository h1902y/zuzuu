...
- **TOON, not JSON** — `notes[3]{addr,type,title,status}:` then one terse line per row. ~40% fewer tokens than JSON for the same rows (no repeated keys, braces, quotes). Every turn the agent spends reading output is a turn it isn't thinking; keep it cheap.
...
## `zz init` — the Project into any repo
...
```bash
zz init                                  # scaffold the Project (git-citizen)
...
`observe` watched real Claude sessions, found a command that recurred, and proposed it as a runnable action. You approved it. Now `zz act` runs it, and a generation pins the moment. The Project grew, from work, through the gate, and you can roll it back. Seven lessons, one working system.

---

**Next:** `08` · The cull — retiring the v1 substrate now that the v2 core replaces it. *(Written when the cull lands.)*
