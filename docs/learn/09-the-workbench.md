...
> Lessons `02`–`08` were the Project: the envelope, the verbs, the loop, the gate,
...
chat loop — it's a real terminal with the Project alongside.
...
**Where this leaves the project:** the Project (lessons `02`–`08`) and the workbench
(this page) are one repo, one CLI (`zz`), one folded web package — the visible
surface over the same envelopes the Project manages, every mutation still flowing
