...
And one rule does a lot of quiet work: **unknown keys are kept, never rejected.** If a miner later wants to add `confidence: 0.8`, it just writes it — old files don't break, the parser preserves it. That's how the Project learns new vocabulary without a migration.
