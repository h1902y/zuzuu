...
Your host coding-agent — **Claude Code / Codex / Gemini CLI** — gives an agent a **brain** (the reasoning loop + the model). **motorsandsensors gives that agent an evolving Memory, body of Knowledge, set of Actions (skills), and Guardrails — improved automatically from how you actually work.**
...
motorsandsensors is **terminal-first** in *experience* and **MCP-core** in *mechanism*, in three rings:
...
2. **Stage 2 — double down on OpenCode (after stage 1 stabilizes):** `mns code` ships as a preconfigured OpenCode bundle (plugin + faculties + guardrails + optional MNS provider/credits) — the one-step door for users who run no host. This is where the **efficiency benchmark** (§2) runs at scale: the observe layer measures token-per-task with/without faculties on cheap models. Not a fork — a distribution.
