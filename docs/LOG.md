...
## Session-merge gate — the workbench surface + the integration-gap fix (U9) (2026-06-27)

Still on `feat/session-merge-gate`. The surfaces (U5–U8) and then a correctness pass (U9) on top of the behavior change above.

**The surfaces (U5–U8).** The held queue got a face on both planes. CLI: `zz session status` enumerates each held session with its merge-review summary (diff numstat + `git merge-tree --write-tree --quiet` mergeability — a **pure read**, working tree + index byte-identical before/after, even when it detects a conflict). Workbench: the **session close card** (`web/src/client/shell/review/`) fires on `heldChanges > 0 OR pending > 0` and surfaces the held diff beside the brain proposals — **Merge** (squash onto `main`, serialized on the main tree per KTD6) / **Discard** — through the daemon's `/api/sessions/held/:id/{merge,discard}` routes (SAFE_ID + live-membership-checked). A conflict routes to **Resolve** (opens a shell at the held worktree). One door still holds: the daemon shells the gated `zz` verb for every land/drop.

**The integration-gap fix (U9) — the hard lesson.** The units (U2–U8) each shipped green in isolation, but a final `/code-review` caught the gate **broken in integration**: the namespace split (active `zz/session-*` vs held `zz/held-*`) and the live-vs-held distinction were inconsistent across the units that met at the seams. Six confirmed bugs, fixed integration-test-first (`tests/unit/session-merge-gate-integration.test.mjs` — failing first, then fixed):

- **F1 (critical) — held in-place sessions were unmergeable.** `closeSession`/`continueSession`/`discardSession` (+ the web routes) resolved targets via `listSessionBranches` (`zz/session-*` only), so an END-held `zz/held-*` branch was unreachable — `zz session merge` returned `no-session-branch` and the work stranded. The verbs now resolve a held target too, taking an optional id (`merge|continue|discard [<id>]`: with an id the specific branch; without, the single active-or-held — ambiguous among several held → a clear refusal). `continue` **re-activates** a held branch (`zz/held-*` → `zz/session-*`) so further checkpoints + a later merge work.
- **F2 — agent-close lost its in-place arm.** The daemon shelled `worktree finalize/close` even for an in-place-fallback agent (`usesWorktree=false`), which no-op'd and stranded the active branch. `agent-close.ts` branches on `usesWorktree` again; in-place → a new `zz session finalize` verb (hold) / `zz session merge` (autoMerge).
- **F3 (critical) — a live agent could be merged mid-session.** `finalizeSessionWorktree` left the held branch unmarked, so `heldSessionBranches` counted **every** worktree-checked-out `zz/session-*` (live agents included) as awaiting merge. Finalize now sets a git-config marker (`branch.<b>.zz-held=true`); a worktree branch counts as held **only with the marker** — a live agent never does.
- **F4 — wrong verb in the hint.** Status + digest printed a blanket `zz session merge` for every held session, orphaning a worktree-held one. The verb is now derived per kind (`heldMergeHint`: in-place → `merge`; worktree → `worktree close <id>`).
- **F5 — `finalizeSession` rename-before-checkout.** It renamed `zz/session-*` → `zz/held-*` **before** checking out base; a failed checkout left the tree stuck on a branch no verb recognized. Reordered: checkout base first, rename only after — on checkout failure it returns without renaming (the active branch survives, recoverable).
- **F6 — `heldAction` over-read.** The web route ran the full diff/mergeability probe just to validate id+kind; now a cheap `listHeldRefs` (namespace + marker, no probe), still SAFE_ID + membership-checked.

**The lesson worth keeping:** all-green per-unit suites hid a whole-feature break at the unit seams — the namespace an earlier unit introduced wasn't taught to the verbs a later unit owned. The fix is cross-unit **integration** tests (end-an-in-place-session-then-merge-it; status-with-a-live-agent-not-counted; finalize-checkout-failure-recoverable), not more per-unit coverage. Verified by hand against the real engine + the published-path CLI (`node bin/zuzuu.mjs session merge` lands a held branch; a live worktree agent is not counted). **CLI 345 / web 505 green** (+6 integration, +4 web).

## The command-registry redesign — one table drives router · help · gate · daemon (2026-06-28)

Landed on `feat/command-registry` (PR #85, 13 commits). The CLI had grown to a **~40-verb flat `switch`** in `cli/index.mjs`, with a hand-maintained `HELP` const beside it and the daemon hand-typing its own argv arrays — three (really four) copies of "what verbs exist," free to drift. They had: the daemon once shelled **nonexistent** verbs (`module new` / `generation mint`, a live bug from the merge-gate work), and the brain-write gate's deny set was a second hand-list nobody guaranteed matched the verbs. The audit's verdict: the verb set is **data**, and every consumer should read the one copy. An 11-rung program (rung 0 = characterize the write paths / session-git contract / gate decisions first, so the refactor stays byte-faithful) rebuilt the surface around a single declarative table — keeping the suite green at every commit.

**The table is the single source of truth.** `src/cli/commands.mjs` is now an array of `{ path, plane, summary, usage, handler, …metadata }` rows, and **four consumers read it**: (a) the router (`cli/index.mjs`, a ~63-line longest-prefix matcher — no logic, just resolve→ctx→handler), (b) `zz help` (folded from the table by plane then namespace, so an unlisted command *can't* exist), (c) the guardrails gate's brain-write **deny set** (`gateWriteVerbPattern` / `writeVerbPaths`, derived from the rows' `permission`/`agentInvokable` — §moat below), and (d) the daemon's **spawn catalog** (`zz commands --json` → `commandCatalog()` → `web/src/server/zuzuu-catalog.ts`, byte-equality pinned by a web test). Add or rename a verb in the table and the change flows to all four for free; the daemon can only shell a command the table knows.

**Two-tier grammar (back-compat is data).** Tier-1 keeps the hot-loop verbs FLAT (querying them every turn must stay cheap on tokens): `init · query · act · check · review · stage · observe · validate · digest · start · wrap · steer · status · doctor · explain · hook`. Tier-2 moves the cold verbs under **noun namespaces** — `note · gen · session · host · registry` — which also resolved the collisions a flat space couldn't: `merge → note fold`, `log → gen log`, the session lifecycle `merge/finalize/continue/discard → land/hold/resume/drop`, `subscribe → registry subscribe`, `enable/disable/code/web → host *`. Every old flat verb survives as a **deprecating alias ROW** (`{ path, alias }`): the router emits ONE stderr deprecation note (kept off stdout so the daemon's JSON.parse stays clean) and dispatches the SAME handler — no per-verb wrapper boilerplate.

**One write boundary — the moat made STRUCTURAL.** `src/grow/commit.mjs` is now the **sole note-writer + sole minter** for the loop: `commit(home, {actor}, batch, opts)` applies a normalized op batch as ONE all-or-nothing transaction (per-op snapshot/restore compensation — a mid-batch failure restores exactly the files it touched, never the old blanket `git clean` that once nuked the seeded-but-uncommitted brain rules), then mints **one generation per touched module**. Before this there were TWO note-writers with divergent invariants — `evolve` (validated, logged, minted) and `refactor` (RAW writes, no validate) — exactly the split a knowledge-poisoning attacker exploits. `rename/merge/refactorField/rollback` are now pure batch **expanders** (decide *what* changes, emit ops) feeding the one writer (decides *how* it lands). Verified structural, not aspirational: `writeNote`/`removeNote` are called from `commit.mjs` and nowhere else (the operator-initiated manifest writes — `init`, the schema alter-table ops, on-demand template mint — bypass `commit` deliberately, each being a gate-event itself, and write `module.md`/`project.md`, never a note). And `commit` **refuses `actor:'agent'`** up front (trust-context, threaded unchanged from the entry boundary — never read from agent-controllable input), so the agent reaches the writer only through `review`.

**Schema enforcement — the dormant FieldType apparatus, now LIVE.** A `module.md` **`fields` block** (the 8 FieldTypes — `text · longtext · select · multi · link · date · number · bool`) is enforced at the write boundary: `repo.writeNote` → `validateNote(note, fields)` rejects a schema-violating note (a missing required column, a value that won't coerce) BEFORE it lands. Absent `fields` ⇒ schemaless (the per-type invariants only — full back-compat). "A module is a TABLE, a note a ROW, the columns are the schema": the web typed-grid and the `module schema/add-column/alter-column/drop-column` ops (operator-gated, each minting a rollback-able generation on the manifest) are the surfaces over it.

**The metal + data layers (the refactor underneath).** `src/metal/{git,fs,sqlite}.mjs` are now the I/O owners — the one git primitive (`{ok,code,out,err}`, **promoted byte-identical** from the session engine so its safety characterization stays exact; `sessions/git.mjs` re-exports it), the byte-I/O wrappers, and the lazy `node:sqlite` loader. `src/notes/repo.mjs` is the note repository (`readNote · writeNote[validates] · removeNote · listNoteIds`) — the one seam where a note becomes bytes; `note.mjs` stays pure. `src/notes/rows.mjs` is the **lossless read projection**: `readEnvelopes`/`searchRows` hydrate the FULL on-disk envelope (every custom frontmatter column survives to the grid — the index row carries only the five indexed columns, so a custom column was invisible through it: the data-loss-in-view, now closed). The module-as-table query (`index.mjs` `plan()`) runs filter (`--text/--type/--tag/--status/--where k=v` EAV) + sort + `--limit/--offset` **server-side** in parameterized SQL — never "fetch everything + filter in JS."

**The moat hardening (the Bash-bypass hole, closed).** The in-process `actor` check stops agent *code* reaching `commit`, but it can't stop the agent shelling `zz <writeverb>` via **Bash** — that fresh process is stamped operator. Rung 9 closes it at PreToolUse: `gateWriteVerbPattern()` (generated from the table — a verb is denied when it durably mutates AND isn't `agentInvokable`, so the sanctioned channels `stage`/`observe` stay allowed) seeds the `protect-brain-exec` rule, and `protect-brain-writes-shell` denies a `cp`/`mv`/redirect INTO `.zuzuu/`. Because both come from the one table, the "agent can't mutate its own brain" invariant **can't drift** — add a write verb and it's denied automatically.

**The web half — full data-op parity + the generated catalog.** The daemon's `zuzuu-catalog.ts` is a typed mirror of the CLI catalog (`commandId` = `keyof COMMAND_CATALOG`, so a typo is a tsc error; an unknown id at runtime is REFUSED) — it can only shell commands the table knows. The client `DataProvider` reaches full op parity over module-as-table: `create/update/delete/deprecate` (target a note) + `relate/unrelate` (carry a relation EDGE — the `link` FieldType's write), every op staging a proposal the review gate governs (THE INVERSION — a UI write is a proposal, not a mutation). `--json` is a single-emit invariant (the post-write integrity nudge rides as a `warnings` field on the document, never a 2nd stdout line that would break JSON.parse).

**The `/ce-review` adversarial pass — three real bugs.** A final review caught, and the last commit fixed: (1) a **silent data-loss** bug in the rollback/revert path; (2) a **session gate hole** — the deny set denied `zz session drop --yes` but ALLOWED its alias `zz session discard --yes` (and `worktree discard`), so the agent could delete a held branch at the merge gate (now the `session` row declares its sub-dispatched mutators as `writeSubcommands`, which the deny set reads); (3) a whitespace/query regression. The lesson reinforced: the moat is only as wide as its *generated* set — an alias the generator doesn't follow is a hole.

**Verified.** Full suite green at the merge: **CLI 435 / web 535** (node:test + vitest, hermetic). The structural claims are grep-checked, not asserted — one note-writer (`commit`), one git primitive (`metal/git`), one catalog the daemon reads. The dropped/deferred parts are unchanged (no OTLP, the async evolution service still design-only).

---

## 2026-07-01 — the ACP drive lane, landed as the Claude Code onboarding default (branch `feat/acp-onboarding-default`, PR #99)

**What shipped.** The validated ACP drive lane — the daemon as the ACP *client* spawning the `claude-agent-acp` adapter, relaying a structured `session/update` stream the workbench renders itself (off the host TUI) — is on the branch and is the **default agent lane for Claude Code**. `laneFor(type, host)` in `web/src/client/shell/session/use-start-session.ts` is the ONE routing seam every start entry point bottoms out in (onboarding host picker · "+ new session" · Overview), so a Claude Code agent lands in the neon ACP conversation while other hosts + plain shells keep the PTY terminal. Ported onto a `main` that had diverged 20 commits (the IA refactor + the neon re-skin); the port was feasibility-verified clean and the small shell wiring re-applied by hand onto the post-refactor structure.

**Live-verified end to end** (personal browser, real Claude Code login): + new session → Claude Code spawned the adapter, `initialize`/`newSession` handshook against the local subscription login, and a prompt streamed a real agent turn back into the conversation ("acp lane works", 31,730 tok, `end_turn`) — all neon.

**Hardened for a default lane** — a 7-persona `ce-doc-review` drove this: the *port* was safe, but *defaulting* promoted the reviewers' concerns to requirements. The gate has no silent-off path (`gateCheck` tells "the gate couldn't run" → a flagged "guardrail unavailable" prompt apart from "no rule matched"); the adapter env is an explicit **allowlist** (was inherit-all-minus-a-few — it forwarded the daemon's whole environment to a third-party adapter); mode selection refuses a blind auto-allow fallback; `create()` no longer leaves a zombie adapter on a failed start; detach arms an idle-grace kill (no orphaned adapter on reload) with `GET /api/acp` liveness reconciling ghost nav rows; the pre-1.0 deps are pinned (the `^1.0.0` caret had already drifted to 1.1.0) with a handshake smoke.

**Deferred (R7b — the default ships neon-coherent without them; full spec in `docs/design-research/workbench/11-conversation-kit-and-entities.md`, grounded in Vercel's Chat SDK + AI Elements):** the conversation-kit graduation into named neon components + a Turn/role grouping (render the user's own prompt) + real Markdown of agent text; the inline onboarding `acpFailure` UI + an up-front per-host "use terminal" toggle. status-aware Stop landed.

**Verified.** `tsc` clean throughout; the ACP server + client + handshake tests green; `ds-no-inline` green. Draft PR #99 pending the deferred UI + a live glance at the onboarding failure flows.
