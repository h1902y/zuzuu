// zuzuu/module/envelope.mjs — the Module Standard envelope (W24).
//
// ONE storage format across all five modules: one file per item, markdown
...
//   module: knowledge        # required, one of the 5
//   kind: command             # required; per-module kinds (MODULE_KINDS)
...
//     - session: ses_abc
//       ref: occurrences=12
//   payload:                  # module-typed machine fields
//     type: command           #   scalar
...
/** Per-module item kinds. `null` = open set (knowledge kinds are governed by
 ...
 *  .zuzuu/<module>/schema.json by `zuzuu init`. We author both ends. */
...
const TOP_KEYS = ['id', 'module', 'kind', 'title', 'status', 'created_at', 'updated_at'];
...
export function parseEnvelope(text) {
  ...
  if (!item.module) errors.push('item missing module');
...
export function validateEnvelope(item, payloadSchema = null) {
  ...
  if (!MODULES.includes(item.module)) errors.push(`module must be one of ${MODULES.join('|')} (got '${item.module}')`);
  const kinds = MODULE_KINDS[item.module];
  if (!item.kind || !/^[a-z0-9][a-z0-9-]*$/.test(item.kind)) errors.push(`kind must be a slug (got '${item.kind}')`);
  else if (Array.isArray(kinds) && !kinds.includes(item.kind)) errors.push(`kind must be one of ${kinds.join('|')} for ${item.module} (got '${item.kind}')`);
