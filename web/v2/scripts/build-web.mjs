...
//
//   web/v2  --tsc(server)+vite(client)-->  dist/{server,shared,web}
...
const v2 = join(dirname(fileURLToPath(import.meta.url)), ".."); // web/v2
const repoRoot = join(v2, "..", "..");
const out = join(repoRoot, "web-app");
const pkg = JSON.parse(readFileSync(join(v2, "package.json"), "utf8"));

function run(cmd, args) {
  const r = spawnSync(cmd, args, { cwd: v2, stdio: "inherit" });
  if (r.status !== 0) {
...
const siSrc = join(v2, "src", "server", "shell-integration");
const siOut = join(v2, "dist", "server", "shell-integration");
...
for (const d of ["server", "shared", "web"]) {
  cpSync(join(v2, "dist", d), join(out, "dist", d), { recursive: true });
}
