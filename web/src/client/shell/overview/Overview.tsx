...
export function Overview(props: OverviewProps) {
  const { name, path, enabled, modules, sessions, onPickModule, onPickSession, onStartSession, onReview } = props;
    ...
    <div className="h-full overflow-y-auto px-10 py-10">
      <div className="mx-auto w-full max-w-5xl">
        <Stack gap="xl">
          {/* identity */}
          <Stack gap="sm">
            <Inline gap="sm"><Icon icon={Database} size={20} /><Text size="2xl" font="display">{name}</Text></Inline>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <Stack gap="sm">
              <Inline gap="xs" justify="between">
                ...
                <Text as="button" interactive size="meta" tone="muted" onClick={onStartSession}>+ new</Text>
              </Inline>
              <Stack gap="xs">
                {cards.map((s) => (
                  <button
                ...
                {!cards.length && (
                  <div className="rounded-ui border border-border bg-surface px-4 py-6 text-center">
                    <Text size="ui" tone="muted">No sessions yet — start one to grow the brain.</Text>
                  </div>
                )}
              </Stack>
            </Stack>
