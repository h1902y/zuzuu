...
// A centered modal over a scrim: type (or autocomplete) an absolute path, ⏎ opens
// it. Reuses the switcher's open-folder reducer + the names-only /api/projects/dir
// route; opening hands off to useEnterProject (switchTo → Overview). Thin .tsx —
// the reducer is the tested logic; the frame uses static layout utilities only.
...
export function NewProject({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    if (!folder.prefix.startsWith("/")) return;
    let alive = true;
      ...
      <div className="animate-pop relative w-full max-w-xl rounded-lg border border-border bg-elevated p-6 shadow-overlay">
        <Stack gap="md">
          ...
          <Inline gap="sm" justify="end">
            ...
            <Button variant="primary" size="sm" disabled={!folder.prefix.startsWith("/")} onClick={() => open(folder.prefix)}>Open</Button>
          </Inline>
        </Stack>
      </div>
    </div>
