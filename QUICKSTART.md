...
**OpenCode** has live capture too — `mns enable --host opencode` installs a `.opencode/plugin/mns.js` that fires on `session.created`/`session.idle` (same lifecycle shape as Claude — no clean end signal, so `mns doctor` reconciles ended/killed sessions).
