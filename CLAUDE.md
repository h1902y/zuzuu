...
**`grow/` — grow the brain, every write human-gated (the loop, all in one place).** `observe.mjs` (aggregate per-session signals across the corroboration threshold and **route** each candidate to the right module — a recurring command → a runnable `action` note, a hot file → a `knowledge` entity), `enhance.mjs` (mine the run log — co-invocation), `propose.mjs` (the typed, deduped, ranked proposal queue), `review.mjs` (**THE human gate** — approve = CRUD + log + mint a generation; the only door to the brain), `snapshot.mjs` (content-addressed per-module **generations**; rollback = pointer-flip + content restore, never `git revert`), `log.mjs` (the append-only event log — the feedback edge).
