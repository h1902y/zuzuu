...
The **workbench** lives at `web/` — a self-contained nested project (own package.json/lockfile, vitest, React/Vite/Hono): `cd web && npm ci && npm test` (103 tests) · `npm run dev` (daemon :7770 + Vite :5173) · root `npm run build:web` stages it into `web-app/` for publishing. Its conventions live in `web/CLAUDE.md`.
