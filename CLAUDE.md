...
**`use/` — use the brain (read · run · inspect).** `query.mjs` (FTS + graph walk, TOON), `act.mjs` (run a runnable note under its policy tier + the `run.allow` command-axis; capture `{stdout,stderr,exit,success}` + log), `check.mjs` (integrity: broken links · orphans · stale). These never mutate the brain.

**`grow/` — grow the brain, every write human-gated (the loop, all in one place).** `observe.mjs` (aggregate per-session signals across the corroboration threshold and **route** each candidate to the right module — a recurring command → a runnable `action` note, a hot file → a `knowledge` entity), `enhance.mjs` (mine the run log — co-invocation), `propose.mjs` (the typed, deduped, ranked proposal queue), `review.mjs` (**THE human gate** — approve = CRUD + log + mint a generation; the only door to the brain), `snapshot.mjs` (content-addressed per-module **generations** + whole-brain **checkpoints**; rollback = pointer-flip, never `git revert`), `log.mjs` (the append-only event log — the feedback edge).
...
**`sessions/` — session = git branch (the most safety-critical area, characterization-first).** `record.mjs` (the lifecycle state machine + the tracked `sessions.json` index), `session-git.mjs` (one session = one `zz/session-*` branch, turn-checkpoints with secret exclusion, squash-merge on end, single-working-branch invariant; fail-soft, never throws), `session-worktree.mjs` (a worktree per session → concurrency), `session-manifest.mjs` (portable content-addressed definition + `restore`), `git.mjs` (plumbing), `labels.mjs`.

**`cli/` — the thin veneer.** A flat router (`index.mjs`) where every verb is a one-liner onto `serve/api`; `init.mjs` (git-citizen scaffold), `session.mjs`, `enable.mjs` (install the hook block by a stable `#zz-hook` signature, never clobbering user hooks), `doctor.mjs` (health · status · explain), `code.mjs` (launch OpenCode as the bundled host), `web.mjs` (the workbench), `migrate.mjs` (v1 home → v2). AXI: TOON output, brief-by-default, no blocking prompts.

**`serve/` — compose · expose · ground.** `api.mjs` (the one façade — `open(cwd)` → a brain handle the CLI + web both consume), `registry.mjs` (the capability dispatch table — `register`/`invoke`, manifest-gated, fail-soft), `wire.mjs` (`registerAll` — binds the verbs; `review` deliberately unregistered, it's the human gate), `digest.mjs` (the deterministic session-start brief).
