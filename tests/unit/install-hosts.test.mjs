...
const cmd = (ev) => `node /x/zuzuu.mjs hook ${ev} --host gemini-cli || true`;
...
test('addHookEntries is idempotent; removeHookEntries strips only ours', () => {
  ...
  assert.equal(twice.hooks.SessionStart.length, 2, 'user hook + one zuzuu hook, not duplicated');
...
test('idempotent + removable with the REAL quoted command form (zuzuu.mjs" hook)', () => {
  ...
  const removed = removeHookEntries(twice);
  assert.equal(removed.hooks, undefined, 'disable removed all zuzuu hooks');
});
