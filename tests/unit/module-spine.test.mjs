// tests/unit/module-spine.test.mjs
// TDD: shared module spine — proposal/provenance/trail/registry (WS2-T1)
...
import { MODULES, moduleDir, inboxDir, proposalsDir, archiveDir } from '../../zuzuu/module/contract.mjs';
import { proposalId, makeProposal, writeProposal, readProposal, listProposals, archiveProposal } from '../../zuzuu/module/proposal.mjs';
import { prov, mergeProvenance } from '../../zuzuu/module/provenance.mjs';
import { recordTrail } from '../../zuzuu/module/trail.mjs';
import { register, get as getAdapter, all as allAdapters } from '../../zuzuu/module/registry.mjs';

// ---------------------------------------------------------------------------
// helpers
// ---------------------------------------------------------------------------
function withHome(fn) {
  const root = mkdtempSync(join(tmpdir(), 'zuzuu-module-'));
...
// contract.mjs
// ---------------------------------------------------------------------------
test('MODULES lists all five module names', () => {
  assert.deepEqual(MODULES, ['knowledge', 'memory', 'actions', 'instructions', 'guardrails']);
});

test('contract path helpers return expected sub-paths', () => {
  const home = '/tmp/.zuzuu';
  assert.equal(moduleDir(home, 'knowledge'), '/tmp/.zuzuu/knowledge');
...
test('makeProposal produces a well-formed pending record', () => {
  ...
  assert.equal(p.module, 'knowledge');
...
test('writeProposal → listProposals returns the proposal; readProposal round-trips', () => {
  withHome((home) => {
    ...
    assert.equal(read.module, 'knowledge');
  });
});
...
test('dual-read: legacy {candidate, er} is normalized to {payload, analysis.er}', () => {
  withHome((home) => {
    ...
    const dir = join(home, module, 'proposals');
    ...
    const r = readProposal(home, module, legacy.id);
    ...
    assert.equal(r.module, module, 'module is set');

    // listProposals also normalizes
    const list = listProposals(home, module);
    assert.equal(list.length, 1);
...
test('dual-read: legacy with both candidate AND payload keeps payload', () => {
  withHome((home) => {
    ...
    const r = readProposal(home, module, mixed.id);
...
test('archiveProposal moves record to archive/ with status + resolved_at', () => {
  withHome((home) => {
    ...
    assert.equal(listProposals(home, module).length, 1);

    const resolved = archiveProposal(home, module, p.id, {
      status: 'approved',
      reason: 'looks good',
    ...
    assert.equal(listProposals(home, module).length, 0);

    // archive file exists
    const archPath = join(home, module, 'proposals', 'archive', `${p.id}.json`);
...
test('archiveProposal with status rejected', () => {
  withHome((home) => {
    const module = 'memory';
    const p = makeProposal({ module, kind: 'item', source: 's', payload: { id: 'x' } });
    writeProposal(home, p);
    const resolved = archiveProposal(home, module, p.id, { status: 'rejected', reason: 'dupe' });
...
test('recordTrail uses the module name for the file (knowledge.jsonl vs actions.jsonl)', () => {
  withHome((home) => {
