...
import { modulesBlock, injectBlock, removeBlock, hasBlock } from '../../zuzuu/home/inject.mjs';
...
test('inject appends the block to existing content, user text intact', () => {
  ...
  assert.match(out, /zuzuu — agent module home/);
});

test('inject into empty text yields just the block', () => {
  const out = injectBlock('');
  assert.match(out, /^<!-- >>> zuzuu:modules:v\d+ >>> -->/);
});

test('re-inject does not duplicate (replaces own block only)', () => {
  const once = injectBlock(USER);
  const twice = injectBlock(once);
  assert.equal((twice.match(/zuzuu:modules:v\d+/g) || []).length, 1);
  assert.ok(twice.startsWith(USER));
});

test('a newer block version replaces an older one in place', () => {
  const v1 = injectBlock(USER, modulesBlock(1)) + '\n## User section after\n';
  const v2 = injectBlock(v1, modulesBlock(2));
  assert.ok(v2.includes('zuzuu:modules:v2'));
  assert.ok(!v2.includes('zuzuu:modules:v1'));
...
test('v4 block carries the ground/cite/harvest contract', () => {
  ...
  assert.ok(out.includes('zuzuu:modules:v4'), 'is v4');
...
test('a v3 block upgrades to v4 in place, user text intact', () => {
  ...
  const v4 = injectBlock(v3, modulesBlock(4));
  assert.ok(v4.includes('zuzuu:modules:v4'));
  assert.ok(!v4.includes('zuzuu:modules:v3'));
  assert.ok(v4.includes('## after'));
});

test('v9 block is the zuzuu marker + points to the digest + zuzuu commands', () => {
  const out = injectBlock('# proj\n');
  assert.ok(out.includes('zuzuu:modules:v9'), 'is v9 zuzuu');
...
test('a v9 zuzuu block re-injects in place (idempotent)', () => {
  ...
  const twice = injectBlock(once);
  assert.equal((twice.match(/zuzuu:modules/g) || []).length, 2, 'one block = begin+end markers only');
