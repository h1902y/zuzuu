...
test('store: a `..`/separator id is rejected; a slug id is allowed', () => {
  ...
  assert.equal(isSafeSegment('../../instructions/items/x'), false);
...
test('review: approving a `../` proposal cannot escape the module or neuter a guardrail', () => {
  withHome(({ home }) => {
    const before = readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8');
    const p = stageChange(home, 'knowledge', { op: 'create', target: '../../instructions/items/no-root-wipe', change: { type: 'rule', action: 'allow', pattern: 'rm -rf' } });
    const r = approve(home, 'knowledge', p.id);
    assert.equal(r.ok, false);
    assert.match(r.error, /unsafe id/);
    assert.equal(readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8'), before, 'guardrail untouched');
  });
});
...
test('review: a `../` relate `from` cannot escape the module either', () => {
  withHome(({ home }) => {
    const before = readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8');
    const p = stageChange(home, 'knowledge', { op: 'relate', change: { from: '../../instructions/items/no-root-wipe', type: 'related-to', to: 'x' } });
    const r = approve(home, 'knowledge', p.id);
    assert.equal(r.ok, false);
    assert.match(r.error, /unsafe id/);
    assert.equal(readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8'), before, 'guardrail untouched');
...
test('review: a relate `to` is stored as a relation VALUE — content, never a write path', () => {
  withHome(({ home }) => {
    ...
    const before = readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8');
    const r = approve(home, 'knowledge', stageChange(home, 'knowledge', { op: 'relate', change: { from: 'src-note', type: 'related-to', to: '../../instructions/items/no-root-wipe' } }).id);
    assert.equal(r.ok, true, 'a weird `to` is accepted — it is content on the FROM note, not a path');
    assert.match(readFileSync(join(home, 'knowledge', 'items', 'src-note.md'), 'utf8'), /\.\.\/\.\.\/instructions/, 'stored verbatim as a relation value');
    assert.equal(readFileSync(join(home, 'instructions', 'items', 'no-root-wipe.md'), 'utf8'), before, 'nothing written outside the module');
  });
});

// ── P0: instructions gate bypass (whitespace-escape, long flags, quoted root) ───

test('gate: root-wipe deny fires on every bypass variant; allows safe deletes', () => {
  withHome(({ home }) => {
    const rules = loadRules(home, 'instructions');
...
test('gate: a ReDoS-shaped rule pattern is rejected at compile (never hangs)', () => {
  withHome(({ home }) => {
    note(home, 'instructions', 'redos', { type: 'rule', action: 'deny', tool: '*', pattern: '(a+)+$' });
    clearCache();
    const rules = loadRules(home, 'instructions');
